...
def merge_scores(K=-1):
    ...
    for model in all_models:
        ...
        all_stat[model][f"haiku_reward.task_macro.K={K}"] = H_TM = (
            haiku_rewards.get(model, {"task_macro_reward": 0})["task_macro_reward"] * 100
        )
        all_stat[model][f"llama_reward.task_macro.K={K}"] = L_TM = (
            llama_rewards.get(model, {"task_macro_reward": 0})["task_macro_reward"] * 100
        )
        all_stat[model][f"gpt4t_reward.task_macro.K={K}"] = G_TM = (
            gpt4t_rewards.get(model, {"task_macro_reward": 0})["task_macro_reward"] * 100
        )
        ...
        for task_tag in all_task_types:
            all_stat[model][f"WB_score.{task_tag}"] = (
                scores.get(model, {"task_categorized_scores": {}})["task_categorized_scores"].get(task_tag, 0) * 10
            )

        all_stat[model][f"WB_score"] = scores.get(model, {"adjusted_score": 0})["adjusted_score"] * 10
        all_stat[model][f"WB_score.task_macro"] = (
            scores.get(model, {"adjusted_task_macro_score": 0})["adjusted_task_macro_score"] * 10
        )
    ...
    for model in all_stat:
        all_stat[model]["Rank_Avg"] = (
            all_stat[model]["Rank_ScoreMacro"] + all_stat[model][f"Rank_TaskMacroReward.K"]
        ) / 2
        all_stat[model]["RewardScore_Avg"] = (
            all_stat[model]["WB_score.task_macro"] + all_stat[model][f"task_macro_reward.K={K}"]
        ) / 2
