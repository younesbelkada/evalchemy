...
def sample_an_eval_result(model_list=[], tag_list=[], eval_mode="score", sample_session_id=None, return_all=False):
    ...
    # print the args
    print(
        f"Model List: {model_list} | Tag List: {tag_list} | Eval Mode: {eval_mode} | Sample Session ID: {sample_session_id}"
    )
    ...
    if eval_mode == "score":
        ...
        for session_id in common_ids:
            ...
            task_tags = [task_group_new[data_item["primary_tag"]]] + [
                task_group_new[x] for x in data_item["secondary_tags"]
            ]
            #     continue
...
if __name__ == "__main__":
    ...
    print(
        sample_an_eval_result(
            model_list=["Llama-3-Instruct-8B-SimPO"],
            tag_list=[
                "Creative Tasks",
                "Planning & Reasoning",
                "Math & Data Analysis",
            ...
            eval_mode="ref=claude-3-haiku-20240307",
        )
    )
